...
const RightPanel = () => {
  ...
  return (
    <div className="flex flex-row w-[50%] h-full bg-white justify-center items-center gap-[7rem]">
      <motion.div
        className="cursor-pointer rounded-full p-2"
        animate={clickedEmoji === 'thumbsup' ? { scale: 1.2, backgroundColor: '#fcd34d' } : {}}
        transition={{ duration: 0.3 }}
        onClick={() => handleClick("thumbsup", "Satisfied")}
      >
        <Emoji name="thumbsup" size={"64"} />
      </motion.div>
      <motion.div
        className="cursor-pointer rounded-full p-2"
        animate={clickedEmoji === 'thinking_face' ? { scale: 1.2, backgroundColor: '#fcd34d' } : {}}
        transition={{ duration: 0.3 }}
        onClick={() => handleClick("thinking_face", "Not Sure")}
      >
        <Emoji name="thinking_face" size={"64"} />
      </motion.div>
      <motion.div
        className="cursor-pointer rounded-full p-2"
        animate={clickedEmoji === 'thumbsdown' ? { scale: 1.2, backgroundColor: '#fcd34d' } : {}}
        transition={{ duration: 0.3 }}
        onClick={() => handleClick("thumbsdown", "Dissatisfied")}
      >
        <Emoji name="thumbsdown" size={"64"} />
      </motion.div>
      <AnimatePresence>
        {label && (
          <motion.div
